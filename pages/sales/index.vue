<template>
  <div class="animate-fade-in">
    <SalesHeading :uploadFile="uploadFile" :uploading="uploading" />
      ...
      <div>
        ...
        <div v-else>
          <SalesList
            ...
            :status="status"
          />
          <SalesEmpty v-else :uploadFile="uploadFile" :uploading="uploading" />
        </div>
      </div>
    </div>
  </div>
...
const fetchDocuments = async () => {
  try {
    let query = { type: "Invoice", status: status.value };
...
const fetchDocumentStats = async () => {
  try {
    const response = await useNuxtApp().$api.documents.stats(companyId, "Invoice");
...
const fetchCustomers = async () => {
  const response = await useNuxtApp().$api.customers.list(companyId, 1, 0);
  ...
  function checkDone() {
    if (completed === validFiles.length) {
      setTimeout(() => {
        ...
        if (succeeded > 0) {
          showToast(succeeded + " invoice" + (succeeded > 1 ? "s" : "") + " uploaded successfully", "success");
        }
        if (failed > 0) {
